...
pub fn Add(a: Matrix(f32), b: Matrix(f32)) MatrixError!void {
    if (a.rows != b.rows or a.cols != b.cols) {
        return MatrixError.MissMatchShape;
    }

    const ctx = backend.GetInstance();
    matrix_add(ctx.context, a.values.ptr, b.values.ptr, a.rows * a.cols);
}

pub fn Subtract(a: Matrix(f32), b: Matrix(f32)) MatrixError!void {
    if (a.rows != b.rows or a.cols != b.cols) {
        return MatrixError.MissMatchShape;
    }

    const ctx = backend.GetInstance();
    matrix_sub(ctx.context, a.values.ptr, b.values.ptr, a.rows * a.cols);
}
...
pub fn Scale(a: Matrix(f32), scale: f32) MatrixError!void {
    const ctx = backend.GetInstance();
    matrix_scale(ctx.context, scale, a.values.ptr, a.rows * a.cols);
}
